...
from brixel.utils.Dataset import GetLargeView, DownsampleLargeView
...
def main():
    ...
    do_normalize = 'dino' in args.backbone_model_name

    model = build_model(args.backbone_model_name, dinov3_weight_path=args.backbone_weight_path,
                        adapter_weight_path=args.adapter_weight_path)

    high_res_transform = GetLargeView(patch_size=model.adapter.backbone.patch_size, do_normalize=do_normalize)
    ...
    for idx, img_path in enumerate(files):
        ...
        with torch.no_grad():
            ...
            if 'finetune' in args.backbone_weight_path:
                out_dir = ensure_dir(os.path.join(args.save_dir, args.backbone_model_name + '_finetuned', str(idx)))
            else:
                out_dir = ensure_dir(os.path.join(args.save_dir, args.backbone_model_name, str(idx)))
