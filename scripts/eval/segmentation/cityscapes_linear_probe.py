...
from scripts.eval.segmentation.ade20k_linear_probe import set_seed, LinearSegProbe, \
    compute_confusion_matrix, train_one_epoch, color_to_tensor
...
def main():
    ...
    p.add_argument("--backbone-model-name", type=str, default='dinov3_vitb16',
                        help="Name of the backbone model.")
    p.add_argument("--backbone-weight-path", type=str,
                        default='data/weights/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth',
                        help="Path to pretrained backbone weights.")
    p.add_argument("--adapter-weight-path", type=str,
                        default='data/weights/dinov3_vitb.pth',
                        help="Path to pretrained backbone weights.")
    p.add_argument("--use-brixel", default=True)

    p.add_argument('--data-root', type=str, default='eval/Cityscapes/')
    p.add_argument('--readout-out-ch', type=int, default=19)
    p.add_argument('--save-dir', type=str, default='results/cityscapes')
    ...
    if args.use_brixel:
        backbone_only = False
        backbone_class = 'dinov3_brixel'
    else:
        backbone_only = True
        backbone_class = 'dinov3'
    backbone = build_model(args.backbone_model_name, dinov3_weight_path=args.backbone_weight_path,
                           adapter_weight_path=args.adapter_weight_path, backbone_only=backbone_only, )
    model = LinearSegProbe(backbone, backbone_class=backbone_class, out_ch=args.readout_out_ch).to(device)
    ...
    if backbone_class == 'dinov3_brixel':
        args.backbone_model_name = args.backbone_model_name + '_brixel'
    if args.img_size != 256:
        args.backbone_model_name = args.backbone_model_name + '_' + str(args.img_size)
    print("I will save as {}.".format(args.backbone_model_name))
