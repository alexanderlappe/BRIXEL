...
class LinearSegProbe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_size: Optional[Tuple[int, int]] = None) -> torch.Tensor:
        with torch.no_grad():
            ...
            if self.backbone_class == "dinov3_brixel":
...
def main():
    ...
    parser.add_argument("--backbone-model-name", type=str, default='dinov3_vitb16',
                        help="Name of the backbone model.")
    parser.add_argument("--backbone-weight-path", type=str,
                        default='data/weights/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth',
                        help="Path to pretrained backbone weights.")
    parser.add_argument("--adapter-weight-path", type=str,
                        default='data/weights/dinov3_vitb.pth',
                        help="Path to pretrained backbone weights.")
    parser.add_argument("--use-brixel", default=True)
    ...
    # Do only evaluation
    parser.add_argument("--head-ckpt", default=None, action="store_true")
    ...
    backbone_name = args.backbone_model_name
