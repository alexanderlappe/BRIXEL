...
from scripts.eval.depth.nyu import seed_everything, human_time, save_json, save_lines, silog_loss
from brixel.models import build_model
from scripts.eval.depth.probes import DepthModel
...
def train_and_eval(args):
    ...
    model = DepthModel(backbone, backbone_class, args.img_size, output_surface_normals=True)
...
def get_args():
    ...
    ap.add_argument("--backbone-model-name", type=str, default='dinov3_vitb16',
                    help="Name of the backbone model.")
    ap.add_argument("--backbone-weight-path", type=str,
                    default='data/weights/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth',
                    help="Path to pretrained backbone weights.")
    ap.add_argument("--adapter-weight-path", type=str,
                    default='data/weights/dinov3_vitb.pth',
                    help="Path to pretrained backbone weights.")
    ap.add_argument("--use-brixel", default=True)


    ap.add_argument("--data-root", type=str, required=False, default="navi_v1.0/",
    ...
    ap.add_argument("--save-dir", type=str, required=False, default="/results/NAVI/")
    return ap.parse_args()


def main():
    args = get_args()
    args.save_dir = os.path.join(args.save_dir, args.backbone_model_name)
    train_and_eval(args)
